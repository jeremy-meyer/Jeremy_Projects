...
def prep_row_to_str(row_list, ntabs=1, inside_sep_char='|'):
    row = list(row_list)
    row.insert(0, '||')
    row.insert(3+1, inside_sep_char)
    row.insert(6+2, inside_sep_char)
    row.append('||')
    tabs = ''.join(['\t' for _ in range(ntabs)])
    return tabs.join([str(x) for x in row])


# Takes a matrix as an input, prints the entire puzzle
def pretty_print_str(m, replace_blanks='_', changed_vals=[], ntabs=1):
    tabs = ''.join(['\t' for _ in range(ntabs)])
    m_str = np.array(m, dtype=str)
    if len(changed_vals)!=0:
        for r,c in changed_vals:
            m_str[r][c] += '*'
    main_puzzle_list = [prep_row_to_str(x, ntabs, inside_sep_char=' ').replace('0', replace_blanks) for x in m_str]
    mid_lines = prep_row_to_str([' ' for x in range(9)], inside_sep_char='+')
    horizontal_edge = tabs.join(['--' for x in range(9+2)])
    main_puzzle_list.insert(3, mid_lines)
    main_puzzle_list.insert(6+1, mid_lines)
    top_border = '//\t' + horizontal_edge + '\t\\\\'
    bottom_border = '\\\\\t' + horizontal_edge+ '\t//'
...
def get_all_possible_values(m, unknown_val=0):
    result = [[{} for x in range(9)] for y in range(9)]
    for r in range(9):
        for c in range(9):
            cell_value = m[r,c]
...
def solve_puzzle(sudoku_matrix, debug=False):
    # Set Up
    iteration = 0
    ...
    while unsolved_cells > 0:
        iteration += 1
        print(f'Starting iteration {iteration} with {unsolved_cells} unsolved cells')
        ...
        for r_c, value in cells_to_assign:
            ...
            print(f'Assigned {r_c} to value {value}')

        unsolved_cells = get_num_empty_spaces(result_matrix)

        if (num_cells_to_assign == 0) and (unsolved_cells>0):
            print('Unable to fully finish puzzle solver')
            break
        elif unsolved_cells == 0:
            print(f'Successfully solved Puzzle after {iteration} iterations!')
        else:
            print(f'After iteration {iteration}, {unsolved_cells} cells left to solve')
        pretty_print_str(result_matrix, changed_vals=changed_cells)

    print(f'Puzzle valid: {verify_sudoku_puzzle(result_matrix)}') # Does it work?
    return_dict = {'result': result_matrix, 'locations_changed': changed_cells}
    if debug:
